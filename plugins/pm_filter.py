...
async def advantage_spoll_choker(bot, query):
    _, user, movie_index = query.data.split('#')
    corrected_movies = SPELL_CHECK.get(query.message.reply_to_message.id)
    if not corrected_movies:
        return await query.answer("The request has already been processed or is old.", show_alert=True)
    if int(user) != 0 and query.from_user.id != int(user):
        return await query.answer("You do not have permission to perform this action.", show_alert=True)
    if movie_index == "close_spellcheck":
        return await query.message.delete()

    corrected_movie = corrected_movies[int(movie_index)]
    await query.answer("In Progress...", cache_time=60)

    files, offset, total_results = await get_search_results(query.message.chat.id, corrected_movie, offset=0, filter=True)
    if files:
        await auto_filter(bot, query, (corrected_movie, files, offset, total_results))
    else:
        reqstr1 = query.from_user.id if query.from_user else 0
        reqstr = await bot.get_users(reqstr1)
        await bot.send_message(chat_id=LOG_CHANNEL, text=f"No result: {reqstr.id}, {reqstr.mention}, {corrected_movie}")
        await query.message.edit("Can't find the movie.")
